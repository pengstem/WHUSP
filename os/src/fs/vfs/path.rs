...
fn lookup_child_raw(cursor: VfsCursor, component: &str) -> Option<VfsCursor> {
    if cursor.kind != FsNodeKind::Directory {
        return None;
    }
...
fn resolve_path_inner(cwd: Option<WorkingDir>, path: &str, mode: LookupMode) -> Option<VfsCursor> {
...
pub(crate) fn resolve_existing(
    ...
    Some(resolve_path_inner(cwd, path, mode)?.as_path())
}

pub(crate) fn resolve_mount_target(cwd: Option<WorkingDir>, path: &str) -> Option<VfsPath> {
...
pub(crate) fn resolve_open(
    ...
    if let Some(existing) = resolve_existing(cwd, path, LookupMode::Normal) {
        return Some(VfsOpenTarget::Existing(existing));
    }

    if !for_create {
        return None;
    }
    Some(VfsOpenTarget::Create(resolve_create_parent(cwd, path)?))
}
