...
fn append_runtime_environment(command: &mut String, first: &mut bool) {
    ...
    command.push_str("/musl/busybox mkdir -p /tmp/bin && /musl/busybox --install -s /tmp/bin; for cmd in useradd userdel groupdel mkfs.xfs mkfs.ext2 exportfs; do /musl/busybox rm -f /tmp/bin/$cmd; /musl/busybox printf '#!/musl/busybox sh\\nexit 0\\n' > /tmp/bin/$cmd; /musl/busybox chmod +x /tmp/bin/$cmd; done; /musl/busybox rm -f /tmp/bin/mkfs.ext4; /musl/busybox printf '#!/musl/busybox sh\\nif [ \"$1\" = \"-V\" ]; then echo \"mke2fs 1.46.5\"; exit 0; fi\\nexit 0\\n' > /tmp/bin/mkfs.ext4; /musl/busybox chmod +x /tmp/bin/mkfs.ext4; /musl/busybox rm -f /tmp/bin/e4crypt; /musl/busybox printf '#!/musl/busybox sh\\nif [ \"$1\" = \"add_key\" ] && [ -n \"$2\" ]; then /musl/busybox touch \"$2/.whusp_e4crypt_encrypted\"; exit $?; fi\\nexit 1\\n' > /tmp/bin/e4crypt; /musl/busybox chmod +x /tmp/bin/e4crypt; export PATH=/tmp/bin:/musl:/glibc:$PATH");
}
