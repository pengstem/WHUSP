...
mod arch {
    ...
    global_asm!(
        r#"
        .section .rodata.vdso_clock,"a"
        .globl __whusp_la_vdso_clock_gettime_start
        .globl __whusp_la_vdso_clock_freq
        .globl __whusp_la_vdso_wall_offset_ns
...
1:
        rdtime.d $t0, $t1
__whusp_la_vdso_freq_pc:
        pcaddi  $t1, 0
        # Keep this PC-relative delta page-layout independent after copying into vDSO.
        addi.d  $t1, $t1, 0x64
        ld.d    $t1, $t1, 0
        beqz    $t1, 2f
        div.du  $t2, $t0, $t1
        ...
        div.du  $t3, $t3, $t1
        beqz    $t5, 3f
__whusp_la_vdso_wall_offset_pc:
        pcaddi  $t6, 0
        # Keep this PC-relative delta page-layout independent after copying into vDSO.
        addi.d  $t6, $t6, 0x40
        ld.d    $t6, $t6, 0
...
2:
        ...
        jr      $ra
        .balign 8
__whusp_la_vdso_clock_freq:
        .dword 0
__whusp_la_vdso_wall_offset_ns:
        .dword 0
__whusp_la_vdso_clock_gettime_end:
        .balign 4
__whusp_la_vdso_gettimeofday_start:
        addi.d  $sp, $sp, -48
        st.d    $a0, $sp, 0
        st.d    $a1, $sp, 8
        st.d    $ra, $sp, 40
        ...
        li.w    $a0, 0
        addi.d  $a1, $sp, 16
        bl      __whusp_la_vdso_clock_gettime_start
        bnez    $a0, 7f
        ld.d    $t0, $sp, 0
        ld.d    $t1, $sp, 16
        ld.d    $t2, $sp, 24
...
__whusp_la_vdso_clock_getres_end:
    ...
    unsafe extern "C" {
        static __whusp_la_vdso_clock_gettime_start: u8;
        static __whusp_la_vdso_clock_freq: u8;
        static __whusp_la_vdso_wall_offset_ns: u8;
    ...
    fn clock_code_size() -> Option<usize> {
        unsafe {
            let start = &__whusp_la_vdso_clock_gettime_start as *const u8 as usize;
            let end = &__whusp_la_vdso_clock_gettime_end as *const u8 as usize;
            end.checked_sub(start)
        }
    }
    ...
    fn gettimeofday_code_offset() -> Option<usize> {
        unsafe {
            let start = &__whusp_la_vdso_clock_gettime_start as *const u8 as usize;
            let gettimeofday = &__whusp_la_vdso_gettimeofday_start as *const u8 as usize;
            gettimeofday.checked_sub(start)
        }
    }
    ...
    fn clock_freq_offset() -> Option<usize> {
        unsafe {
            let start = &__whusp_la_vdso_clock_gettime_start as *const u8 as usize;
            let freq = &__whusp_la_vdso_clock_freq as *const u8 as usize;
            freq.checked_sub(start)
        }
    }
    ...
    fn wall_offset_in_code() -> Option<usize> {
        unsafe {
            let start = &__whusp_la_vdso_clock_gettime_start as *const u8 as usize;
            let wall_offset = &__whusp_la_vdso_wall_offset_ns as *const u8 as usize;
            wall_offset.checked_sub(start)
        }
    }
    ...
    fn write_elf_header(image: &mut [u8], phoff: usize, phentsize: usize, phnum: usize) {
        ...
        image[6] = 1;
        write_u16(image, 16, ET_DYN);
        write_u16(image, 18, EM_LOONGARCH);
        write_u32(image, 20, EV_CURRENT);
        write_u64(image, 32, phoff as u64);
        write_u16(image, 52, 64);
        write_u16(image, 54, phentsize as u16);
    ...
    fn write_version_def(image: &mut [u8], off: usize, name: usize) {
        ...
        write_u16(image, off + 4, 2);
        write_u16(image, off + 6, 1);
        write_u32(image, off + 8, elf_hash(LINUX_5_10));
        write_u32(image, off + 12, 20);
        write_u32(image, off + 16, 0);
        write_u32(image, off + 20, name as u32);
        write_u32(image, off + 24, 0);
