#!/usr/bin/env python3
"""Run the feature-gated ReadMostlySnapshot concurrency probe."""
...
PROBE_SOURCE = REPO_ROOT / "scripts" / "perf-gates" / "read_mostly_snapshot_probe.c"
GUEST_TEMPLATE = REPO_ROOT / "scripts" / "perf-gates" / "read-mostly-snapshot-probe.sh"
CASES = ("parallel_readers", "publish_grace_period", "nested_reader")
...
def build_probe(args: argparse.Namespace, temp_root: Path, setup: Path) -> Path:
    output = temp_root / "read_mostly_snapshot_probe"
    command = [
...
def build_disk(args: argparse.Namespace, temp_root: Path, probe: Path, setup: Path) -> Path:
    ...
    entry.write_text(
        "#!/musl/busybox sh\nexec /musl/busybox ash /x1/read-mostly-snapshot-probe.sh || exit 127\n",
        encoding="utf-8",
    )
    entry.chmod(0o755)
    guest = staging / "read-mostly-snapshot-probe.sh"
    guest.write_text(render_guest(args), encoding="utf-8")
    guest.chmod(0o755)
    installed = staging / "read_mostly_snapshot_probe"
    shutil.copy2(probe, installed)
    installed.chmod(0o755)
    image = temp_root / "read-mostly-snapshot-probe.img"
    output: list[str] = []
    try:
...
def validate(log: str, args: argparse.Namespace, overlay_root: Path) -> dict[str, Any]:
    ...
    errors: list[str] = []
    identity = (
        f"run_id={args.run_id} arch={args.arch} smp={args.smp} mem={args.mem} "
        f"block_io={args.block_io_mode}"
    )
    if lines.count(f"READ_MOSTLY_GUEST_START {identity}") != 1:
        errors.append("guest start marker mismatch")
    if lines.count(f"READ_MOSTLY_GUEST_PASS {identity}") != 1:
        errors.append("guest pass marker mismatch")
    seen = {
        line.removeprefix("READ_MOSTLY_CASE_PASS case=")
        for line in lines
        if line.startswith("READ_MOSTLY_CASE_PASS case=")
    }
    if seen != set(CASES):
        errors.append(f"case matrix mismatch: {sorted(seen)}")
    if lines.count("READ_MOSTLY_PROBE_PASS cases=3") != 1:
        errors.append("probe pass marker mismatch")
    if any("READ_MOSTLY_CASE_FAIL" in line or "READ_MOSTLY_FAIL" in line for line in lines):
...
def run(args: argparse.Namespace) -> int:
    ...
    setup.mkdir()
    temp_root = Path(tempfile.mkdtemp(prefix="whusp-g0b-read-mostly-"))
    ...
    print(f"ReadMostlySnapshot probe {'PASS' if sample['valid'] else 'FAIL'} arch={args.arch}")
    if not sample["valid"]:
...
def parse_args() -> argparse.Namespace:
    ...
    if args.smp < 8 or args.smp > bench.MAX_CPUS:
        parser.error(f"--smp must be in 8..{bench.MAX_CPUS}")
