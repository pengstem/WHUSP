#!/usr/bin/env python3
"""Exercise S0-2 dirty-overlay pressure reclaim and verify its batch bounds."""
...
PROBE_SOURCE = REPO_ROOT / "scripts" / "perf-gates" / "s0-2_dirty_pressure_probe.c"
GUEST_TEMPLATE = REPO_ROOT / "scripts" / "perf-gates" / "s0-2-dirty-pressure-probe.sh"
...
def build_probe(args: argparse.Namespace, temp_root: Path, setup: Path) -> Path:
    output = temp_root / "s0-2_dirty_pressure_probe"
    command = [
...
def build_disk(args: argparse.Namespace, temp_root: Path, probe: Path, setup: Path) -> Path:
    ...
    entry.write_text(
        "#!/musl/busybox sh\nexec /musl/busybox ash /x1/s0-2-dirty-pressure-probe.sh || exit 127\n",
        encoding="utf-8",
    )
    entry.chmod(0o755)
    guest = staging / "s0-2-dirty-pressure-probe.sh"
    guest.write_text(render_guest(args), encoding="utf-8")
    guest.chmod(0o755)
    installed = staging / "s0-2_dirty_pressure_probe"
    shutil.copy2(probe, installed)
    installed.chmod(0o755)
    image = temp_root / "s0-2-dirty-pressure-probe.img"
    output: list[str] = []
    try:
        bench.run_setup_command([bench.require_command("truncate"), "-s", "96M", str(image)], output)
...
def perf_snapshot(lines: list[str], point: str) -> tuple[dict[str, int], list[str]]:
    begin = f"S0_2_DIRTY_PRESSURE_PERF_BEGIN point={point}"
    end = f"S0_2_DIRTY_PRESSURE_PERF_END point={point}"
...
def validate(log: str, args: argparse.Namespace, overlay_root: Path) -> dict[str, Any]:
    ...
    errors: list[str] = []
    start = (
        f"S0_2_DIRTY_PRESSURE_GUEST_START run_id={args.run_id} arch={args.arch} "
        f"smp={args.smp} mem={args.mem} block_io={args.block_io_mode} perf=1"
    )
    passed = (
        f"S0_2_DIRTY_PRESSURE_GUEST_PASS run_id={args.run_id} arch={args.arch} "
        f"smp={args.smp} mem={args.mem} block_io={args.block_io_mode} perf=1"
    )
    required = (
    ...
    candidates = delta.get("vfs_dirty_overlay_pressure_candidates", 0)
    batch_inodes = after.get("vfs_dirty_overlay_pressure_batch_inodes_max", 0)
    batch_pages = after.get("vfs_dirty_overlay_pressure_batch_pages_max", 0)
    budget_stops = delta.get("vfs_dirty_overlay_pressure_budget_stops", 0)
    if candidates != 16:
        errors.append(f"pressure candidate bound mismatch: {candidates}")
    if not 1 <= batch_inodes <= 16:
        errors.append(f"pressure inode batch out of range: {batch_inodes}")
    if batch_pages != 256:
        errors.append(f"pressure page batch mismatch: {batch_pages}")
    if budget_stops != 1:
        errors.append(f"pressure budget stop mismatch: {budget_stops}")
    if delta.get("vfs_dirty_writeback_pressure_flushed_pages", 0) != 256:
        errors.append("pressure reclaim did not flush exactly 256 pages")
    if delta.get("vfs_dirty_writeback_pressure_flush_failures", 0) != 0:
        errors.append("pressure reclaim reported a writeback failure")
    if delta.get("vfs_dirty_overlay_global_scan_calls", 0) != 0:
        errors.append("candidate used the retired whole-map scan")
    if after.get("vfs_dirty_writeback_dirty_pages", -1) != 0:
        errors.append("dirty pages did not drain after sync")
    if after.get("vfs_dirty_writeback_dirty_files", -1) != 0:
        errors.append("dirty inode queue did not drain after sync")
    return {
        "valid": not errors,
        "errors": errors,
        "perf_before": before,
...
def run(args: argparse.Namespace) -> int:
    ...
    setup.mkdir()
    temp_root = Path(tempfile.mkdtemp(prefix="whusp-g0b-s0-2-dirty-pressure-"))
    ...
    try:
        ...
        if process["returncode"] != 0 or process["timed_out"]:
            errors.append(
                f"QEMU failed returncode={process['returncode']} timeout={process['timed_out']}"
            )
    ...
    print(f"S0-2 dirty-pressure probe {'PASS' if sample['valid'] else 'FAIL'} arch={args.arch}")
    if not sample["valid"]:
